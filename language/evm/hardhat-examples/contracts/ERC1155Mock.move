...
module Evm::ERC1155Mock {
    ...
    #[external(sig=b"onERC1155Received(address,address,uint256,uint256,bytes)")]
    public native fun IERC1155Receiver_call_onERC1155Received(contract: address, operator: address, from: address, id: U256, amount: U256, bytes: vector<u8>): Result<vector<u8>, vector<u8>>;

    #[external(sig=b"onERC1155BatchReceived(address,address,uint256[],uint256[],bytes)")]
    public native fun IERC1155Receiver_call_onERC1155BatchReceived(contract: address, operator: address, from: address, ids: vector<U256>, amounts: vector<U256>, bytes: vector<u8>): Result<vector<u8>, vector<u8>>;
...
//        doSafeTransferAcceptanceCheck(operator, from, to, id, amount, data);
    ...
    /// Transfers `_value` amount of an `_id` from the `_from` address to the `_to` address specified (with safety call).
    public fun safeBatchTransferFrom(from: address, to: address, ids: vector<U256>, amounts: vector<U256>, _data: vector<u8>) acquires State {
        require(to != @0x0, b"ERC1155: transfer to the zero address");
    ...
    public fun supportsInterface(interfaceId: vector<u8>): bool {
        ...
        let id165 = IERC165_interfaceId(); //0x01ffc9a7
        let id1155 = IERC1155_interfaceId(); //0xd9b67a26
        ...
        // ) ||
        // (*Vector::borrow(&interfaceId, 0) == 217 &&
        //     *Vector::borrow(&interfaceId, 1) == 182 &&
        //     *Vector::borrow(&interfaceId, 2) == 122 &&
        //     *Vector::borrow(&interfaceId, 3) == 38
        // )
    }
    ...
    //     if (isContract(to)) {
    //         let result = IERC1155Receiver_call_onERC1155Received(to, operator, from, id, amount, data);
    //         if (Result::is_ok(&result)) {
    //             let retval = Result::unwrap(result);
    //             let expected = IERC1155Receiver_selector_onERC1155Received();
    //             require(retval == expected, b"error_message");
    //         }
    //         else {
    //             let _error = Result::unwrap_err(result);
    //             abort_with(b"error_message") // TODO: abort with the `_error` value.
    //         }
    //     }
    // }
    ...
    //     if (isContract(to)) {
    //         let result = IERC1155Receiver_call_onERC1155BatchReceived(to, operator, from, ids, amounts, data);
    //         if (Result::is_ok(&result)) {
    //             let retval = Result::unwrap(result);
